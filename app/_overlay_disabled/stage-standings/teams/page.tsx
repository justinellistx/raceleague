...
type Row = { pos: number; team: string; pts: number }

export default function OverlayStageStandingsTeams() {
  const searchParams = useSearchParams()
  const stage = Number(searchParams.get('stage') || '1')

  const [title, setTitle] = useState(`STAGE ${stage} TEAM STANDINGS`)
  const [rows, setRows] = useState<Row[]>([])
  const [error, setError] = useState<string | null>(null)

  const load = async () => {
    setError(null)
    setTitle(`STAGE ${stage} TEAM STANDINGS`)
    ...
    const sorted = (data ?? [])
      .map((r: any) => ({
        team: r.team ?? 'Team',
        pts: Number(r.team_stage_points ?? 0) || 0,
      }))
  ...
  return (
    <main style={{ padding: 24, background: 'transparent', fontFamily: 'system-ui' }}>
      <div style={{ width: 520, borderRadius: 16, padding: 16, background: 'rgba(10,10,12,0.92)', color: '#fff' }}>
        ...
        {rows.length === 0 && !error && <div style={{ opacity: 0.7 }}>Waiting for stage team standings…</div>}

        {rows.map((r) => (
          <div key={`${r.pos}-${r.team}`} style={{ display: 'grid', gridTemplateColumns: '50px 1fr 90px', gap: 8, padding: '6px 0' }}>
